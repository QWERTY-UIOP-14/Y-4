  ...
  onStart: async function ({ event, api, commandName }) {
    ...
    const formData = {
      input: {
        ...
        idempotence_token: uuid + "_FEED", // Fixed key name
        source: "WWW",
        attachments: [],
        audience: {
          privacy: {
            allow: [],
            base_state: "FRIENDS", // Adjust as needed
            deny: [],
  ...
  onReply: async function ({ Reply, event, api, commandName }) {
    ...
    if (type === "whoSee") {
      if (!["1", "2", "3"].includes(body)) {
        return api.sendMessage('Please choose one of the three options above', threadID, messageID);
      }
        ...
        for (const attach of attachments) {
          try {
            if (attach.type === "photo") {
              ...
              await fs.writeFile(imagePath, Buffer.from(getFile));
              ...
              await new Promise((resolve, reject) => {
                ...
                videoFile.data.pipe(writeStream);
      ...
      api.httpPost('https://www.facebook.com/api/graphql/', form, async (e, info) => {
        api.unsendMessage(handleReply.messageID);
        try {
          if (e) throw e;
...
async function cleanUpFiles() {
  try {
    await fs.unlink(`${__dirname}/cache/imagePost.png`);
    await fs.unlink(`${__dirname}/cache/videoPost.mp4`);
  } catch (e) {
