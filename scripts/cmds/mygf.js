...
module.exports = {
    config: {
        ...
        countDown: 5,
        role: 0,
        shortDescription: "Purpose Your gf",
        longDescription: "Purpose Your gf",
        category: "love",
        guide: {
            vi: "{pn} @tag ",
            en: "{pn} @tag "
    ...
    onStart: async function ({ message, args, event, api }) {
        ...
        if (mention.length == 0) return message.reply("🔰 Please Mention Your Gf 🔰");
        else if (mention.length == 1) {
            const one = event.senderID, two = mention[0];
            bal(one, two).then(ptth => { message.reply({ body: "  ⊙────𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝙼𝚈 𝙶𝙵────⊙", attachment: fs.createReadStream(ptth) }) })
        } else {
...
async function bal(one, two) {
    ...
    img.resize(1280, 716).composite(avone.resize(360, 360), 130, 200).composite(avtwo.resize(360, 360), 787, 200);
